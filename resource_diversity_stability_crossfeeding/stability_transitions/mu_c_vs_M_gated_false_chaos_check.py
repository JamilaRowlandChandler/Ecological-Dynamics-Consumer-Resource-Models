...
Zoom in on M = 100, 250 at mu_C = 250 (p=0.999, gated=False), 5 communities
each, for both shared_network settings - exploring the marginal/chaotic
dynamics found near M=150, mu_C=250 in mu_c_vs_M_gated_false_p0999.py
(positive Lyapunov exponent but no divergence).
"""
...
for shared_network in [False, True]:
    ...
    save_dir = f"resource_diversity_stability_crossfeeding/chaos_check_shared_{shared_network}"
...
for shared_network in [False, True]:
    ...
    full_location = "C:/Users/jamil/Documents/PhD/Data/" + \
        f"resource_diversity_stability_crossfeeding/chaos_check_shared_{shared_network}"

    df = pd.concat([pd.read_csv(full_location + "/" + file, index_col=False)
                   for file in os.listdir(full_location)],
                   axis = 0, ignore_index = True)

    print(f"--- shared_network={shared_network}, p=0.999, mu_C=250 ---")
    print(df[['M', 'mu_c', 'Max. lyapunov exponent', 'Divergence measure']].sort_values(['M']))
