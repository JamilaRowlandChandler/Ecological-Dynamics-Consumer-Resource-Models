...
Same as diversity_low_M_investigation_o_sweep.py, but with p_s FIXED at 0.5
for every M (instead of scaling as min(5/M,1)).

For each M and condition:
  - the metabolic network (w, q, shared across all consumers) is resampled
    n_network_resamples=3 times
  - for each network, no_communities=20 independent sets of
    growth/consumption rates (c_ia, y_ia) are sampled (network held fixed)
  - for each of those 20 communities, every o in o_values is simulated with
    ...
    vector - isolating the pure effect of increasing influx on a fixed
    community.

M=[5,10,15,20], p_s=0.5 (fixed), K_m=1e-2, d=0.1, mu_C=40 (mu_c=mu_C/M),
S=50, sigma_C=1.6, o=[0.1,0.3,0.5,0.7,0.9,1.1].
"""
...
if __name__ == '__main__':
    ...
    for M_idx, M in enumerate(M_values):
        ...
        for cond_idx, condition in enumerate(['single', 'all']):
            ...
            for net_idx in range(n_network_resamples):
                ...
                network_seed = 810000 + M_idx * 10000 + cond_idx * 1000 + net_idx
                ...
                for community_idx in range(no_communities):
                    ...
                    rate_seed = 860000 + M_idx * 100000 + cond_idx * 10000 \
    ...
    with open('diversity_low_M_investigation_ps05_o_sweep_results.pkl', 'wb') as f:
        pickle.dump(results, f)

    print("Saved results to diversity_low_M_investigation_ps05_o_sweep_results.pkl")
