...
Repeat of diversity_low_M_investigation.py, except the metabolic network is
no longer resampled for every community. For each (M, condition, o)
parameter set, a single set of resource energies w and a single metabolic
matrix q_{alpha, beta} (shared across all consumers, via metabolic_network's
...
M=[5,10,15,20], o=[0.1,0.3,0.5,0.7,0.9,1.1], no_communities=20,
p_s=min(5/M,1) (scaled with M), K_m=1e-4, d=0.1, mu_C=40 (mu_c=mu_C/M), S=50,
sigma_C=1.6 - matching diversity_low_M_investigation.py's settings exactly
except for the network-sharing scheme.
"""
    ...
    for M_idx, M in enumerate(M_values):
        ...
        for cond_idx, condition in enumerate(['single', 'all']):
            ...
            for o_idx, o_val in enumerate(o_values):
                ...
                network_seed = 600000 + M_idx * 10000 + cond_idx * 1000 + o_idx
