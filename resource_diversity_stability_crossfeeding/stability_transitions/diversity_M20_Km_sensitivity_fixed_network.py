...
Repeat of diversity_M20_Km_sensitivity.py, except the metabolic network is no
longer resampled for every community. For each (K_m, condition, o) parameter
...
sampled per community. This isolates how much of the previous run's
community-to-community noise came from network variation vs rate variation.

M=20 fixed, o=[0.1,0.3,0.5,0.7,0.9,1.1], no_communities=20, p_s=min(5/M,1)=0.25,
d=0.1, mu_C=40 (mu_c=mu_C/M), S=50, sigma_C=1.6, K_m in [1e-3, 1e-2, 1e-1] -
matching diversity_M20_Km_sensitivity.py's settings exactly except for the
...
if __name__ == '__main__':
    ...
    for K_m_idx, K_m in enumerate(K_m_values):
        ...
        for cond_idx, condition in enumerate(['single', 'all']):
            ...
            for o_idx, o_val in enumerate(o_values):
                ...
                network_seed = 500000 + K_m_idx * 10000 + cond_idx * 1000 + o_idx
    ...
    with open('diversity_M20_Km_sensitivity_fixed_network_results.pkl', 'wb') as f:
        pickle.dump(results, f)

    print("Saved results to diversity_M20_Km_sensitivity_fixed_network_results.pkl")
