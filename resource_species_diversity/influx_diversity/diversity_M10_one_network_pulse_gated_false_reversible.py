...
o=0 otherwise), but using the new saturation_kinetics='reversible' variant
(growth_saturation=True) instead of the default 'flux' - see
metabolic_network()'s docstring for the formula:
f(R_alpha, R_beta) = (R_alpha - R_beta*exp(-(w_alpha-w_beta))) /
(K_m + R_alpha + R_beta), replacing the bare R_alpha factor in
growth/consumption/production alike. Unlike 'thermodynamic', f depends only
on R (not N_i), so doesn't share that variant's species-count-driven
stiffness - smoke-tested stable and fast (<1s/sim) across the full R_star
range at this M=10/S=50/non-gated scale before this full run.
...
if __name__ == '__main__':
    ...
    out_path = os.path.join(DATA_DIR, 'diversity_M10_one_network_pulse_gated_false_reversible_results.pkl')
